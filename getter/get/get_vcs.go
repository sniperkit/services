...
func vcsByCmd(cmd string, gitreq *cache.Request) vcsProvider {
	switch cmd {
	case "git":
...
type vcsPath struct {
	...
	check  func(ctx context.Context, match map[string]string, gitreq *cache.Request) error // additional checks
...
func (g *Getter) repoRootFromVCSPaths(ctx context.Context, importPath, scheme string, insecure bool, vcsPaths []*vcsPath) (*repoRoot, error) {
	...
	// if the gitcache has a custom resolver, use this
	if r := g.gitreq.Resolver(); r != nil {
		ropo, root, err := r.Resolve(ctx, importPath)
		if err != nil {
			return nil, err
		}
		return &repoRoot{
			vcs:  vcsByCmd("git", g.gitreq),
			repo: ropo,
			root: root,
		}, nil
	}
...
func noVCSSuffix(ctx context.Context, match map[string]string, gitreq *cache.Request) error {
	repo := match["repo"]
...
func bitbucketVCS(ctx context.Context, match map[string]string, gitreq *cache.Request) error {
...
// branch one level up.
func launchpadVCS(ctx context.Context, match map[string]string, gitreq *cache.Request) error {
